...
def test_build_quote_from_snapshot_skips_invalid_sides():
    ...
    # zero bid
    assert build_quote_from_snapshot(ct, types.SimpleNamespace(bid_price=0.0, ask_price=2.0, bid_size=1, ask_size=1, timestamp=ts)) is None
    # zero ask
    assert build_quote_from_snapshot(ct, types.SimpleNamespace(bid_price=1.0, ask_price=0.0, bid_size=1, ask_size=1, timestamp=ts)) is None
    # crossed
    assert build_quote_from_snapshot(ct, types.SimpleNamespace(bid_price=2.1, ask_price=2.0, bid_size=1, ask_size=1, timestamp=ts)) is None
    # zero sizes
    assert build_quote_from_snapshot(ct, types.SimpleNamespace(bid_price=1.0, ask_price=2.0, bid_size=0, ask_size=1, timestamp=ts)) is None
    # None quote -> None
...
def test_build_quote_from_snapshot_produces_valid_quote():
    ct = contract()
    ts = datetime(2026, 8, 30, tzinfo=UTC)
    lq = types.SimpleNamespace(bid_price=2.0, ask_price=2.1, bid_size=5, ask_size=9, timestamp=ts)
...
def test_build_quote_from_stream_event_requires_event_time():
    ...
    assert build_quote_from_stream_event(ct, {"bid": 2.0, "ask": 1.0, "bid_size": 1, "ask_size": 1}, event_time=evt_ts) is None
...
def test_store_record_get_and_recency():
    ...
    store.record(build_quote_from_snapshot(q, types.SimpleNamespace(bid_price=1.0, ask_price=1.1, bid_size=1, ask_size=1, timestamp=datetime(2026, 1, 1, tzinfo=UTC))))
...
def test_record_to_contract_from_contract_master():
    ...
    assert ct.underlying == "A" and ct.right == OptionRight.CALL and ct.multiplier == 100
...
def test_stream_handler_records_into_store():
    ...
    master = ContractMaster()
    master.upsert(OptionContractRecord("AAPL260131C00300000", "AAPL", date(2026, 1, 31), 300.0, "call", 100, 100, "active", True, "american"))
    store = QuoteStore()
    handler = stream_handler(store, master)
    evt_ts = datetime(2026, 8, 30, 12, tzinfo=UTC)
    event = MarketEvent("quote", "AAPL260131C00300000", evt_ts, evt_ts, 1, {"bid": 2.0, "ask": 2.1, "bid_size": 5, "ask_size": 9})
    handler(event)
    assert len(store) == 1
    q = store.get("AAPL260131C00300000")
    assert q is not None and q.bid == 2.0 and q.asof == evt_ts
    # Unknown symbol: handler must not raise.
    handler(MarketEvent("quote", "UNKNOWN", evt_ts, evt_ts, 2, {"bid": 2.0, "ask": 2.1, "bid_size": 5, "ask_size": 9}))
    assert len(store) == 1