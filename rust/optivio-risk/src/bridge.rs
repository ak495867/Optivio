...
pub fn validate(frame: &[u8], previous_sequence: Option<u64>) -> Result<Header, FrameError> {
    ...
    let timestamp_ns =
        u64::from_le_bytes(frame[16..24].try_into().map_err(|_| FrameError::TooShort)?);
    let payload_len =
        u32::from_le_bytes(frame[24..28].try_into().map_err(|_| FrameError::TooShort)?);
    ...
    if previous_sequence.is_some_and(|previous| sequence <= previous) {
        return Err(FrameError::NonMonotonicSequence);
    }
    Ok(Header {
        kind,
        sequence,
