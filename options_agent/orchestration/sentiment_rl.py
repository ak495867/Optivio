...
class CausalSentimentAggregator:
    ...
    def aggregate(
        self, observations: list[SentimentObservation], asset: str, cutoff: datetime
    ) -> tuple[float, float]:
        usable = [
            o
            for o in observations
            if o.asset == asset
            and o.usable_at(cutoff)
            and o.confidence >= self.minimum_confidence
        ]
        if not usable:
            return 0.0, 0.0
...
class ConstrainedPortfolioRL:
    ...
    def act(
        ...
        proposed = (
            regime_scale
            * (1.0 + 0.15 * np.clip(sentiment_score, -1, 1))
            * (1.0 - 0.5 * uncertainty_penalty)
        )
        scale = float(np.clip(proposed, self.min_scale, self.max_scale))
        return RLAction(
            scale, float(max(0.0, entropy)), "bounded regime/sentiment policy"
        )
