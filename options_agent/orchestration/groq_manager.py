...
class GroqManager:
    ...
    def assess_alt_data(self, text: str, source_ids: list[str], asof_iso: str) -> AltDataAssessment:
        if self.client is None:
            self.connect()
        schema = AltDataAssessment.model_json_schema()
        prompt = ("Interpret only the supplied text. Do not infer facts beyond it. "
        ...
        response = self.client.chat.completions.create(model=self.model, messages=[{"role": "user", "content": prompt}], temperature=0, response_format={"type": "json_object"})
