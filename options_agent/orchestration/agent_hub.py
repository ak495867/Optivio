...
class IndependentAgentHub:
    ...
    def __init__(self, agents: Mapping[str, Callable[[object], Signal]], workers: int | None = None):
        if not agents:
            raise ValueError("at least one agent is required")
        self._agents = dict(agents)
        self._pool = ThreadPoolExecutor(max_workers=workers or len(agents), thread_name_prefix="optivio-agent")

    def infer(self, observation: object) -> tuple[AgentResult, ...]:
        futures = {self._pool.submit(agent, observation): agent_id for agent_id, agent in self._agents.items()}
