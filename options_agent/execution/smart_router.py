...
class SmartRouter:
    ...
    def choose(self, quotes: list[Quote], intent: OrderIntent) -> RouteDecision | None:
        ...
        for q in quotes:
            if q.contract.symbol != intent.contract.symbol or min(q.bid_size, q.ask_size) < intent.quantity:
                continue
            mid = (q.bid + q.ask) / 2
            ...
            score = -cost
            candidate = RouteDecision(q.contract.symbol, px, cost, score, "best available size/spread quote", perf_counter_ns() - t0)
