...
class LivePaperLoop:
    ...
    def push_quotes(self, quotes: list[Quote]) -> bool:
        ...
        now = self.clock()
        if any(
            (now - q.available_at).total_seconds() > self.max_quote_age_seconds
            for q in quotes
        ):
    ...
    def process_once(
        self,
        intent_factory: Callable[[list[Quote], RiskSnapshot], Iterable[OrderIntent]],
        snapshot: RiskSnapshot,
        submit: Callable[[OrderIntent], object],
    ) -> int:
