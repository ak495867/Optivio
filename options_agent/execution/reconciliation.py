...
class BrokerReconciler:
    ...
    def reconcile(self, packages: Mapping[str, MultiLegPackage], broker: BrokerSnapshot) -> ReconciliationResult:
        expected_orders = {leg.client_id: leg.filled for package in packages.values() for leg in package.legs}
        ...
        order_drift = any(abs(actual_orders.get(order_id, -1) - qty) > self.quantity_tolerance for order_id, qty in expected_orders.items())
        expected_positions: dict[str, int] = {}
        for package in packages.values():
            for leg in package.legs:
                expected_positions[leg.symbol] = expected_positions.get(leg.symbol, 0) + leg.side * leg.filled
        symbols = set(expected_positions) | set(broker.positions)
        drift = {symbol: broker.positions.get(symbol, 0) - expected_positions.get(symbol, 0) for symbol in symbols if abs(broker.positions.get(symbol, 0) - expected_positions.get(symbol, 0)) > self.quantity_tolerance}
