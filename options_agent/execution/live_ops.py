...
class ContinuousReconciliation:
    ...
    def __init__(self, reconciler: BrokerReconciler, snapshot_provider: Callable[[], tuple[dict[str, MultiLegPackage], BrokerSnapshot]], interval_seconds: float = 5.0):
    ...
    def check_once(self) -> LiveCheck:
        try:
            ...
            ok = result.ok
            detail = "broker state synchronized" if ok else "material broker drift; exposure blocked"
...
def alpaca_paper_stream_smoke_test(symbols: list[str], seconds: float = 5.0) -> dict[str, object]:
    ...
    if not symbols or seconds <= 0 or seconds > 60:
        return {"ok": False, "reason": "symbols required and seconds must be between 0 and 60"}
    try:
    ...
    supervisor = AlpacaStreamSupervisor(factory, subscribe, symbols, lambda: (key, secret), max_retries=2)
    ...
    return {"ok": supervisor.health.state == "connected" or received > 0, "received": received, "paper": True, "stream_state": supervisor.health.state}
