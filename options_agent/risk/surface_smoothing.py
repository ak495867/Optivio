...
def smooth_iv_surface(
    ...
    iv: np.ndarray,
    bandwidth_moneyness: float = 0.15,
    bandwidth_maturity: float = 0.20,
    ...
    valid = (
        np.isfinite(k) & np.isfinite(t) & np.isfinite(values) & (values > 0) & (t > 0)
    )
    ...
    for i in range(len(values)):
        if not valid[i]:
            continue
        weights = np.exp(
            -0.5 * ((k[valid] - k[i]) / max(bandwidth_moneyness, 1e-8)) ** 2
            - 0.5
            * ((np.sqrt(t[valid]) - np.sqrt(t[i])) / max(bandwidth_maturity, 1e-8)) ** 2
        )
        if np.sum(weights) <= 0:
            continue
        result[i] = float(
            np.clip(np.sum(weights * values[valid]) / np.sum(weights), 1e-6, 8.0)
        )
    return result
