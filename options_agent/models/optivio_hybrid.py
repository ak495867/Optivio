...
class KronosStyleTokenizer:
    ...
    def transform(self, x: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        coarse = (
            np.floor(z * self.levels[0]).astype(np.int64).clip(0, self.levels[0] - 1)
        )
...
class HiveStyleRingEncoder:
    ...
    def __init__(
        self, feature_dim: int, hidden_dim: int = 16, neighbors: int = 3, seed: int = 7
    ):
...
class OptivioHybridModel:
    ...
    def __init__(self, feature_dim: int, hidden_dim: int = 16, seed: int = 7):
        ...
        self.encoder = HiveStyleRingEncoder(
            feature_dim + 2, hidden_dim=hidden_dim, seed=seed
        )
    ...
    def fit(
        self, x: np.ndarray, y: dict[str, np.ndarray], ridge: float = 1e-2
    ) -> OptivioHybridModel:
        if x.ndim != 4:
        ...
        design = np.concatenate([emb, np.ones((*emb.shape[:2], 1))], axis=-1).reshape(
            -1, emb.shape[-1] + 1
        )
    ...
    def predict(self, x: np.ndarray) -> HybridOutput:
        ...
        return HybridOutput(
            direction=np.tanh(head("direction")),
            expected_move=np.maximum(0, head("expected_move")),
            volatility=np.maximum(1e-6, head("volatility")),
            liquidity=1 / (1 + np.exp(-head("liquidity"))),
            embedding=emb,
        )
