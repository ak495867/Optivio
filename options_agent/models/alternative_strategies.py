...
class SurfaceArbitrageModel:
    ...
    def put_call_parity(
        ...
        return ArbitrageSignal(
            "put_call_parity",
            abs(edge),
            min(1.0, abs(edge) / max(self.min_edge, 1e-9) / 5),
            ("call", "put", "underlying"),
            "parity residual exceeds threshold",
        )

    def vertical_monotonicity(
        self,
        lower_mid: float,
        higher_mid: float,
        lower_strike: float,
        ...
        return ArbitrageSignal(
            ...
            float(violation),
            min(1.0, violation / 0.10),
            ("lower_strike", "higher_strike"),
            "higher-strike call is priced above lower-strike call",
        )
...
class GaussianHMM:
    ...
    def fit(self, observations: np.ndarray) -> GaussianHMM:
        ...
        self.means = np.quantile(x, np.linspace(0.15, 0.85, self.states))
        self.variances = np.full(self.states, max(float(np.var(x)), 1e-6))
        self.transition = np.full(
            (self.states, self.states), 0.15 / max(self.states - 1, 1)
        )
        np.fill_diagonal(self.transition, 0.85)
        self.initial = np.full(self.states, 1 / self.states)
        for _ in range(self.iterations):
            emissions = self._emissions(x)
            alpha = np.zeros((len(x), self.states))
    ...
    def _emissions(self, x: np.ndarray) -> np.ndarray:
        ...
        return np.exp(-0.5 * (x[:, None] - self.means) ** 2 / self.variances) / np.sqrt(
            2 * np.pi * self.variances
        )
    ...
    def filtered_probabilities(self, observations: np.ndarray) -> np.ndarray:
        ...
        x = np.asarray(observations, dtype=float).reshape(-1)
        emissions = self._emissions(x)
        probs = np.zeros((len(x), self.states))
        probs[0] = self.initial * emissions[0]
        probs[0] /= probs[0].sum() + 1e-12
        for t in range(1, len(x)):
            probs[t] = (probs[t - 1] @ self.transition) * emissions[t]
            probs[t] /= probs[t].sum() + 1e-12
        return probs
