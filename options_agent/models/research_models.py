...
class HiveAdapter:
    ...
    def transform_features(self, bars: pd.DataFrame) -> np.ndarray:
        ...
        out["log_return"] = out.groupby("symbol")["close"].transform(lambda s: np.log(s).diff())
        out["rsi10"] = out.groupby("symbol")["log_return"].transform(lambda s: s.rolling(10, min_periods=10).mean())
...
class KronosAdapter:
    ...
    def load(self):
        try:
            ...
            raise ImportError("make the Kronos repository available or install its dependencies") from exc
    ...
    def forecast(self, bars: pd.DataFrame, future_timestamps: pd.Series, pred_len: int):
        ...
        return self.predictor.predict(df=x, x_timestamp=bars["asof"], y_timestamp=future_timestamps, pred_len=pred_len, T=1.0, top_p=0.9, sample_count=1)
