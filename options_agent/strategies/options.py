...
def vix_regime_signal(vix: float, vix_sma: float, asof: datetime, symbol: str) -> Signal:
    score = 1.0 if vix < vix_sma else -1.0
    confidence = min(0.95, 0.5 + abs(vix - vix_sma) / max(vix_sma, 1e-9))
    return Signal(symbol=symbol, asof=asof, score=score, confidence=confidence, expected_move=abs(vix - vix_sma) / 100, rationale="VIX relative to its point-in-time moving average", features={"vix": vix, "vix_sma": vix_sma})


def select_liquid_contract(quotes: list[Quote], signal: Signal, cfg: StrategyConfig) -> Quote | None:
    now = signal.asof
    eligible: list[Quote] = []
    for q in quotes:
        ...
        if cfg.min_dte <= dte <= cfg.max_dte and spread <= cfg.max_spread_pct and min(q.bid_size, q.ask_size) > 0:
            eligible.append(q)
    if not eligible:
        return None
...
def build_order(signal: Signal, quote: Quote, risk: RiskSnapshot, cfg: StrategyConfig, model_version: str) -> OrderIntent | None:
    ...
    return OrderIntent(client_order_id=f"oa-{quote.contract.symbol}-{int(signal.asof.timestamp())}", contract=quote.contract, side=side, quantity=1, limit_price=round(mid, 2), rationale=signal.rationale, model_version=model_version, signal_asof=signal.asof, created_at=signal.asof)
