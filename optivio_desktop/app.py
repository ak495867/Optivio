...
class OptivioConsole(tk.Tk):
    ...
    def _build_styles(self) -> None:
        ...
        style.configure(
            "TLabel", background="#0b1220", foreground="#c9d5e8", font=("Segoe UI", 10)
        )
        style.configure(
            "Title.TLabel",
            background="#0b1220",
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            header, text="local paper-trading control console", style="Muted.TLabel"
        ...
        ttk.Entry(setup, textvariable=self.alpaca_key, show="•").pack(
            fill="x", pady=(4, 10)
        )
        ttk.Label(setup, text="Alpaca paper secret").pack(anchor="w")
        ttk.Entry(setup, textvariable=self.alpaca_secret, show="•").pack(
            fill="x", pady=(4, 10)
        )
        ttk.Label(setup, text="Groq key (optional, kept advisory)").pack(anchor="w")
        ttk.Entry(setup, textvariable=self.groq_key, show="•").pack(
            fill="x", pady=(4, 10)
        )
        ...
        ttk.Button(
            controls, text="Start / Run", style="Accent.TButton", command=self._start
        ).pack(fill="x", pady=3)
        ttk.Button(controls, text="Pause exposure", command=self._pause).pack(
            fill="x", pady=3
        )
        ttk.Button(controls, text="Recover / Reconnect", command=self._recover).pack(
            fill="x", pady=3
        )
        ttk.Button(
            controls, text="Stop", style="Danger.TButton", command=self._stop
        ).pack(fill="x", pady=3)
        ttk.Label(
            setup,
            text="Secrets are validated in memory and never written to the audit timeline.",
            style="Muted.TLabel",
            wraplength=260,
        ...
        self.component_tree = ttk.Treeview(
            components, columns=("group", "status"), show="headings", height=13
        )
        ...
        for component in self.orchestrator.components():
            self.component_tree.insert(
                "", "end", iid=component.name, values=(component.group, "standby")
            )
        ttk.Button(components, text="Invoke selected", command=self._invoke).pack(
            anchor="e", pady=(10, 0)
        )
    ...
    def _start(self) -> None:
        credentials = RuntimeCredentials(
            self.alpaca_key.get(), self.alpaca_secret.get(), self.groq_key.get()
        )
        mode = RuntimeMode(self.mode.get())
        self._write("Start requested: running preflight; secrets omitted")
        threading.Thread(
            target=self._start_worker, args=(credentials, mode), daemon=True
        ).start()
    ...
    def _start_worker(self, credentials: RuntimeCredentials, mode: RuntimeMode) -> None:
        ...
        if ok and mode == RuntimeMode.CONSTRAINED_PAPER:
            self._write(
                "Constrained paper mode is blocked until verified broker synchronization"
            )
    ...
    def _refresh(self) -> None:
        ...
        self.status.set(
            f"{snapshot.state.value} · mode={snapshot.mode.value} · health checks={snapshot.counters.get('health_checks', 0)} · orders={snapshot.metrics.get('orders', 'standby')} · pnl={snapshot.metrics.get('pnl', 'standby')}"
        )
